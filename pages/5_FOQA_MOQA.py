...
# Per-aircraft drill-down spec: (label, flag column, recorded-value column, AMM limit, recommended action)
...
DRILLDOWN_SPECS = [
    ...
    ("VMO (kias)",           'vmo_exceedance',            'max_cas_kias',            VMO_KIAS,      "review ATA 05 task"),
    ("VLE gear (kias)",      'vle_exceedance',            'max_cas_gear_down',       VLE_KIAS,      "review ATA 05 task"),
    ("APU EGT (°C)",         'apu_egt_exceedance',        'max_apu_egt',             None,          "MPP7166_05-50-55"),
]
...
def drilldown_counts_by_msn(df_in, ac_col):
    ...
    flag_cols = [f for _, f, _, _, _ in DRILLDOWN_SPECS if f in df_in.columns]
    if not flag_cols:
        return pd.Series(dtype=int)
...
def build_drilldown_events(df_in, ac_col, msn):
    ...
    rows = []
    for label, flag_col, value_col, limit, action in DRILLDOWN_SPECS:
...
st.title("🔍 FOQA / MOQA — ATA 05 Exceedance & Engine Trends")
...
if 'date' in df.columns and df['date'].notna().any():
    ...
    if age_days <= 3:
        ...
        st.warning(f"⚠️ No new flights ingested in {age_days} days — pipeline may be stale (latest {latest_flight.date():%d-%b-%Y})")
else:
...
c2.metric("✈️ Hard landings", n_hard)
c3.metric("🔴 N2 vib amber+", n_vib)
c4.metric("🌡️ ITT exceedances", n_itt)
c5.metric("💨 VMO events", n_vmo)

st.divider()

# ── Fleet Exceedance Heatmap ──────────────────────────────────────────────────
st.subheader("🗺️ Fleet Exceedance Heatmap")
...
st.subheader("🔎 Per-Aircraft Exceedance Drill-Down")
...
tab_eng, tab_acft, tab_apu, tab_events = st.tabs(["🔥 Engine Trends", "✈️ Aircraft Exceedances", "⚙️ APU", "📋 Event Log"])


# ── ENGINE TRENDS ─────────────────────────────────────────────────────────────
with tab_eng:
    st.subheader("🎯 Margin to AMM Limit — Engine Triage (fleet-wide)")
    ...
    if df_margin.empty:
        ...
        if not flagged.empty:
            ...
            st.warning(f"⚠️ Engines at or near an AMM limit: {items}")
        else:
            st.success("✅ All engines have a healthy margin to their AMM limit.")

    st.divider()

    st.subheader("ITT — Inter-Turbine Temperature (°C)")
    ...
    st.subheader("Oil Pressure — Min in Flight (psig)")
...
with tab_acft:
    st.subheader("Hard Landing Events")
    ...
    st.subheader("VMO / MMO Exceedances")
...
with tab_apu:
    ...
    if 'max_apu_egt' in df.columns and df['max_apu_egt'].notna().any():
        ...
        if not apu_exc.empty:
            st.warning(f"⚠️ {len(apu_exc)} APU EGT exceedance event(s) — check AMM task MPP7166_05-50-55")
...
# ── EVENT LOG ─────────────────────────────────────────────────────────────────
with tab_events:
    st.subheader("Full Exceedance Log")
    if 'any_exceedance' in df.columns:
        ...
        if not df_exc.empty:
            ...
            st.download_button("⬇️ Export CSV", df_exc.to_csv(index=False), f"foqa_exceedances_{date.today()}.csv", "text/csv")
        else:
